...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { YolkenTest13bInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw YolkenTest13bInvalidDataException("`$name` is not set")
            is JsonNull -> throw YolkenTest13bInvalidDataException("`$name` is null")
            else -> throw YolkenTest13bInvalidDataException("`$name` is invalid, received $this")
        }
